  ...
  <div class="page-header">
    ...
    <div class="header-actions">
      ...
      <button class="btn-add" data-bs-toggle="modal" data-bs-target="#masteradd">
        ...
        <span class="add-txt">Add Payment Term</span>
      </button>
    </div>
  </div>
...
<!-- Add Masters -->
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <form id="createPaymentTermForm" method="POST" action="{{ route('payment_term_store') }}">
            @csrf
            <div class="modal-body">
            <div class="section-block p-2"> 
                <div class="form-group">
                  <label class="form-label">Payment Term Name <span class="text-danger">*</span></label>
                  <input type="text" class="form-input" name="name" required placeholder="Payment Term Name"/>
                </div>
                <div class="text-end">
  ...
  <div class="filter-bar">
    ...
    <select class="filter-select" id="categoryFilter">
      <option value="">Payment Terms: All</option>
  ...
  <div class="table-card">
    <div class="table-scroll">
      <table class="supplier-table" id="PaymentTermTable">
        <thead>
          <tr>
...
<div class="modal fade" id="editPaymentTermModal" tabindex="-1" aria-labelledby="masteraddLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Edit Payment Term</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
        <form  method="POST" action="{{ route('payment_term_updated') }}">
		 @csrf
...
$(document).ready(function () {
    ...
    let table = $('#PaymentTermTable').DataTable({
        ...
        ajax: {
            ...
            url: "{{ route('payment_term_list_table') }}",
            ...
            data: function (d) {
                ...
                d._token = "{{ csrf_token() }}";
                d.payment_term_id = $('#categoryFilter').val();
...
$(document).ready(function() {
  $(document).on('click', '.edit_model_btn', function() {
        var table = $('#PaymentTermTable').DataTable();
	    var rowData = table.row($(this).closest('tr')).data();
	    var payment_term_id = rowData.payment_term_id;
	    // console.log('Delete ID:', payment_term_id);

        $.ajax({
            url: 'payment_term_edit/' + payment_term_id, 
            type: "GET",
            data: {
                "_token": "{{ csrf_token() }}" 
            },
...
$(document).on('click', '.deleteRoleBtn', function () {
    ...
    let payment_term_id = $(this).data('id');
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.status == true) {
                        ...
                        $('#PaymentTermTable').DataTable().ajax.reload();
...
<script>
 $(document).ready(function () {      
   $('#createPaymentTermForm').validate({
       rules: {
           name: {
               required: true
           }
